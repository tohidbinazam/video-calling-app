...
export const userRegister = async (req, res, next) => {
  try {
    ...
    sentMail(
      user.email,
      'Verify Account',
      `Please verify Your account by click this <a href=${
        req.protocol
      }://${req.get('host')}/${verify_link}>LINK</a>`
    );
  ...
  try {
    ...
    if (user) {
      ...
      sentMail(
        user.email,
        'Reset Password',
        `Reset Your Password by click this <a href=${req.protocol}://${req.get(
          'host'
        )}/${reset_pass_link}>LINK</a>. Expire in 10min`
      );
